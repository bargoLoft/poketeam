        ...
        if (call.name === 'replace_file_content' || call.name === 'multi_replace_file_content') {
          ...
          if (targetFile && targetFile.includes('PartyDashboard.jsx')) {
             if (call.name === 'replace_file_content') {
               const target = parseArg(args.TargetContent);
               const repl = parseArg(args.ReplacementContent);
               if (content.includes(target)) {
                 content = content.replace(target, repl);
               } else {
                 console.log("Failed target snippet:", target.substring(0, 100).replace(/\n/g, '\\n'));
               }
             } else {
               ...
               for (const chunk of chunks) {
                 if (content.includes(chunk.TargetContent)) {
                   ...
                   console.log("Failed chunk target snippet:", chunk.TargetContent.substring(0, 100).replace(/\n/g, '\\n'));
                 }
               }
             }
          }
